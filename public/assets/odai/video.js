...
var Video = function () {
    ...
    var view_tbl;
    var view_url = 'videos';
    var list_url = 'getVideo';
    var video_id = document.getElementById('video-id').value;
    console.log(list_url + '/' + video_id);

    /////////////////// View //////////////////
    ///////////////////////////////////////////
    var viewTable = function () {

        var link = list_url + '/' + video_id;
        var columns = [
            {"data": "index", "title": "#", "orderable": false, "searchable": false},
            {"data": "link", "orderable": true, "searchable": false},
            {"data": "episode", "orderable": true, "searchable": true},
            {"data": "part", "orderable": true, "searchable": true},
        ...
        view_tbl = DataTable.init($('#video_table'), link, columns, order, ajaxFilter, perPage);
    };
    ...
    var add = function () {
        $('#add-video').on('click', function (e) {
            let form = document.getElementById('add-new-video');
    ...
    var updateVideo=function () {
        $('#update-video').click(function (e){
            e.preventDefault();
            let form = document.getElementById('update-data_video');
            var formData = new FormData(form);
            let method = form.getAttribute('method');
            let link = form.getAttribute('action');
            Forms.doAction(link, formData, method, null, addCallBack);
